...
function TopicRow({ t, done, grasp, notes, times, timer, toggleDone, setTopicGrasp, setTopicNote, startTimer, stopTimer, color }) {
  ...
  return (
    <div className={`trow${isDone?' done':''}`}
      style={{background:gInfo?`${gInfo.color}05`:'transparent'}}>
      ...
      <button onClick={()=>{
        ...
        aria-label={`Mark "${t.text}" as ${isDone?'incomplete':'complete'}`}
        style={{width:20,height:20,borderRadius:5,marginTop:2,flexShrink:0,cursor:'pointer',
          border:`2px solid ${isDone?color:'rgba(34,211,238,.15)'}`,
          background:isDone?`${color}20`:'transparent',
          display:'flex',alignItems:'center',justifyContent:'center',
          transition:'all .15s'}}>
        {isDone&&<span style={{color,fontSize:11,fontWeight:900,lineHeight:1}}>✓</span>}
      </button>

      <div style={{flex:1,minWidth:0}}>
        <div className="ttext" onClick={()=>toggleDone(t.id)} role="button" tabIndex={0}
          onKeyDown={e=>e.key==='Enter'&&toggleDone(t.id)}
          style={{fontSize:13,color:isDone?'var(--t3)':'var(--t1)',lineHeight:1.55,
            marginBottom:8,cursor:'pointer',transition:'color .15s',userSelect:'none'}}>
          {t.text}
        </div>

        <div className='topic-actions' style={{display:'flex',alignItems:'center',gap:5,flexWrap:'wrap'}}>
          ...
          <button onClick={e=>{e.stopPropagation();isActive?stopTimer(t.id):startTimer(t.id)}}
            ...
            {isActive ? `⏸ ${fmtMs(elapsed)||'0s'}` : timeMs>0 ? `⏱ ${fmtMs(timeMs)}` : '▶ Start'}
          </button>

          {hasNote && <span aria-label="Has note" style={{fontSize:9,color:'var(--t3)'}}>📝</span>}
        </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:4}}>
            <button onClick={()=>toggleDone(t.id)}
              style={{padding:'4px 11px',borderRadius:99,cursor:'pointer',fontSize:10,fontWeight:700,
                border:`1px solid ${done[t.id]?t.phaseColor:'var(--border2)'}`,
                background:done[t.id]?`${t.phaseColor}22`:'transparent',
                color:done[t.id]?t.phaseColor:'var(--t2)'}}>
      ...
      <div className="wrap">
        ...
        <main className="content" id="main" aria-label="Main content" tabIndex={-1}>
          ...
          <Suspense fallback={<div style={{minHeight:'40vh',display:'flex',alignItems:'center',justifyContent:'center'}}><div className="spinner"/></div>}>
            ...
            {view==='community'&&<div className="page-enter"><Community user={user} done={done} totalTopics={stats.total}/></div>}
