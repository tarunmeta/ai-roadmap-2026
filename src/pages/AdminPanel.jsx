...
Happy learning! 🚀`)
  ...
  if (!authed) return (
    <div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,.88)',
      ...
      <div style={{background:'#0d0f1a',border:'1px solid #242740',borderRadius:20,padding:32,maxWidth:380,width:'100%',animation:'popIn .3s cubic-bezier(.16,1,.3,1)'}}>
        <div style={{textAlign:'center',marginBottom:24}}>
          <div style={{fontSize:36,marginBottom:10}}>🔐</div>
          <div style={{fontSize:18,fontWeight:800}}>Admin Panel</div>
          <div style={{fontSize:12,color:'#8b90b0',marginTop:4}}>Powered by Firebase</div>
        </div>
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button onClick={onClose} style={{padding:'12px 16px',borderRadius:10,border:'1px solid #242740',background:'transparent',color:'#8b90b0',fontSize:13,cursor:'pointer'}}>Cancel</button>
        </div>
        <div style={{fontSize:10,color:'#454869',marginTop:10,textAlign:'center'}}>Change in AdminPanel.jsx → ADMIN_PASSWORD</div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,.88)',backdropFilter:'blur(12px)',overflowY:'auto',padding:16}}>
      <div style={{maxWidth:900,margin:'0 auto'}}>
        ...
        {/* Header */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,padding:'14px 20px',background:'#0d0f1a',border:'1px solid #242740',borderRadius:14}}>
          <div>
          ...
          <button onClick={onClose} style={{padding:'7px 14px',borderRadius:8,border:'1px solid #242740',background:'transparent',color:'#8b90b0',fontSize:12,cursor:'pointer'}}>✕ Close</button>
        </div>

        {/* Stats */}
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:14}}>
          {[['🔑','Total Keys',totalCount,'#6366f1'],['✅','Activated',usedCount,'#10b981'],['⏳','Unused',totalCount-usedCount,'#f59e0b'],['💰','Revenue',`₹${revenue.toLocaleString('en-IN')}`, '#25d366']].map(([em,lb,v,c])=>(
            <div key={lb} style={{background:'#0d0f1a',border:`1px solid ${c}22`,borderRadius:12,padding:'14px',textAlign:'center'}}>
        ...
        {/* Generate key */}
        <div style={{background:'#0d0f1a',border:'1px solid #242740',borderRadius:14,padding:20,marginBottom:14}}>
          <div style={{fontSize:14,fontWeight:700,marginBottom:14}}>⚡ Generate Access Key — After Payment</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'flex-end'}}>
            <div style={{flex:1,minWidth:160}}>
              <div style={{fontSize:11,color:'#8b90b0',marginBottom:5}}>Plan</div>
            ...
            <div style={{flex:2,minWidth:200}}>
              <div style={{fontSize:11,color:'#8b90b0',marginBottom:5}}>Buyer name / phone (for your reference)</div>
            ...
            <button onClick={generate} disabled={fbLoading}
              style={{padding:'10px 20px',borderRadius:9,border:'none',cursor:fbLoading?'wait':'pointer',
                background:fbLoading?'#1a1d2e':'linear-gradient(135deg,#6366f1,#a855f7)',
                color:'#fff',fontSize:13,fontWeight:700,whiteSpace:'nowrap',
                boxShadow:fbLoading?'none':'0 4px 16px rgba(99,102,241,.3)'}}>
          ...
          {newKey && (
            <div style={{marginTop:14,padding:'16px',borderRadius:12,
              ...
              border:'1px solid rgba(16,185,129,.25)',animation:'fadeUp .3s ease'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                ...
                <button onClick={()=>copy(newKey,'nk')}
                  style={{padding:'8px 14px',borderRadius:8,border:'1px solid #242740',
                    background:copied==='nk'?'rgba(16,185,129,.15)':'transparent',
                    color:copied==='nk'?'#10b981':'#8b90b0',fontSize:12,fontWeight:700,cursor:'pointer'}}>
        ...
        {/* Keys table */}
        <div style={{background:'#0d0f1a',border:'1px solid #242740',borderRadius:14,overflow:'hidden',marginBottom:14}}>
              ...
              {['all','unused','used'].map(f=>(
                <button key={f} onClick={()=>setFilter(f)}
                  style={{padding:'4px 12px',borderRadius:99,cursor:'pointer',
                    border:`1px solid ${filter===f?'#6366f1':'#242740'}`,
                    background:filter===f?'rgba(99,102,241,.12)':'transparent',
                    color:filter===f?'#818cf8':'#8b90b0',fontSize:11,fontWeight:700}}>
                  {f.charAt(0).toUpperCase()+f.slice(1)}
                </button>
              ))}
              <button onClick={refresh} disabled={fbLoading}
                style={{padding:'4px 10px',borderRadius:8,border:'1px solid #242740',background:'transparent',color:'#8b90b0',fontSize:11,cursor:'pointer'}}>
                {fbLoading?'⏳':'↺'}
              </button>
            </div>
          </div>
          ...
          {keyList.length===0 ? (
            <div style={{padding:'40px',textAlign:'center',color:'#8b90b0',fontSize:13}}>
                ...
                <thead>
                  <tr style={{borderBottom:'1px solid #1a1d2e'}}>
                    {['Key','Plan','Buyer','Created','Status','Actions'].map(h=>(
                      <th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:10,fontWeight:700,color:'#454869',textTransform:'uppercase',letterSpacing:'1px',whiteSpace:'nowrap'}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {keyList.map(([k,v])=>{
                    ...
                    return (
                      <tr key={k} style={{borderBottom:'1px solid rgba(26,29,46,.5)',background:isUsed?'rgba(16,185,129,.02)':'transparent'}}>
                        ...
                        <td style={{padding:'12px 14px'}}>
                          <span style={{fontSize:10,padding:'3px 8px',borderRadius:99,
                            background:`${plan?.color||'#6366f1'}18`,color:plan?.color||'#6366f1',
                            fontWeight:700,border:`1px solid ${plan?.color||'#6366f1'}33`}}>
                            {plan?.name||v.planId}
                          </span>
                        </td>
                        <td style={{padding:'12px 14px',fontSize:11,color:'#8b90b0',maxWidth:130}}>
                          <span style={{display:'block',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                            {v.label||'—'}
                          </span>
                        </td>
                        <td style={{padding:'12px 14px',fontSize:11,color:'#8b90b0',whiteSpace:'nowrap'}}>{fmtDate(v.createdAt)}</td>
                        <td style={{padding:'12px 14px'}}>
                          {isUsed
                            ? <span style={{fontSize:10,padding:'3px 8px',borderRadius:99,background:'rgba(16,185,129,.1)',color:'#10b981',fontWeight:700,border:'1px solid rgba(16,185,129,.2)'}}>✓ Used</span>
                        ...
                        <td style={{padding:'12px 14px'}}>
                          <div style={{display:'flex',gap:4}}>
                            <button onClick={()=>copy(k,k)}
                              style={{padding:'4px 9px',borderRadius:7,border:'1px solid #242740',background:copied===k?'rgba(16,185,129,.1)':'transparent',color:copied===k?'#10b981':'#8b90b0',fontSize:10,fontWeight:700,cursor:'pointer'}}>
                              {copied===k?'✓':'📋'}
                            </button>
                            {!isUsed&&(
        ...
        {/* Workflow */}
        <div style={{padding:'14px 18px',borderRadius:12,background:'rgba(99,102,241,.05)',border:'1px solid rgba(99,102,241,.15)'}}>
          <div style={{fontSize:12,fontWeight:700,color:'#818cf8',marginBottom:6}}>📋 Your daily workflow:</div>
          <div style={{fontSize:12,color:'#8b90b0',lineHeight:1.9}}>
