  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:5,marginTop:8,justifyContent:'flex-end'}}>
        ...
        {['var(--s3)','rgba(0,229,255,.2)','rgba(0,229,255,.4)','rgba(0,229,255,.65)','var(--cyan)'].map((c,i)=>(
          <div key={i} style={{width:11,height:11,borderRadius:3,background:c,border:`1px solid ${c==='var(--s3)'?'var(--border)':'transparent'}`}}/>
        ))}
...
export default function StatsPage({ store, user }) {
  ...
  const SC = ({v,lbl,sub,color,icon}) => (
    <div style={{background:'var(--bg-glass)',border:`1px solid ${color}18`,borderRadius:14,
  ...
  const Section = ({title,action,children}) => (
    <div style={{background:'var(--bg-glass)',border:'1px solid var(--border)',borderRadius:16,padding:'18px 20px'}}>
  ...
  return (
    <div className="fade-up" style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(148px,1fr))',gap:10}}>
        ...
        <SC v={daysLeft>365?'∞':`${daysLeft}d`} lbl="Est. Finish" sub="at current pace" color="var(--cyan)"  icon="🏁"/>
        <SC v={grp.strong}  lbl="Strong"      sub="fully understood"                color="var(--cyan)"  icon="💪"/>
      ...
      <div style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:12}} className="two-col">
        ...
        <Section title="🧠 Grasp Quality">
          <div style={{display:'flex',justifyContent:'center',marginBottom:16}}>
            <Ring pct={doneN?Math.round(grp.strong/doneN*100):0} size={110} stroke={11} color="var(--cyan)">
              <div style={{textAlign:'center'}}>
                <div style={{fontFamily:'Outfit,sans-serif',fontSize:19,fontWeight:800,color:'var(--cyan)'}}>
          ...
          {[['💪 Strong',grp.strong,'var(--cyan)'],['🤔 Mid',grp.mid,'var(--amber)'],
            ...
            <div key={l} style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
              ...
              <div style={{flex:2,height:5,background:'var(--s3)',borderRadius:3,overflow:'hidden'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:9}}>
            {phaseStats.map(p=>(
              <div key={p.id} style={{display:'flex',alignItems:'center',gap:9}}>
                ...
                <div style={{flex:1,position:'relative',height:18,background:'var(--s3)',borderRadius:9,overflow:'hidden'}}>
                  {/* stacked grasp */}
                  <div style={{position:'absolute',inset:0,display:'flex'}}>
                    {p.strong>0&&<div style={{width:`${p.strong/p.total*100}%`,background:'var(--cyan)',opacity:.7}}/>}
                  ...
                  {p.pct>12&&<span style={{position:'absolute',left:8,top:'50%',transform:'translateY(-50%)',
                    fontSize:8,fontWeight:800,color:'var(--void)'}}>
                    {p.done}/{p.total}
                  </span>}
                </div>
                <span style={{fontSize:9.5,fontWeight:800,color:p.pct===100?'var(--cyan)':p.color,width:32,textAlign:'right',flexShrink:0}}>{p.pct}%</span>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{height:4,background:'var(--s3)',borderRadius:2,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${(t.time/maxT)*100}%`,
                        background:timeTab==='slowest'?'var(--red)':'var(--cyan)',
                        borderRadius:2,transition:'width .6s'}}/>
                    </div>
                  </div>
                  <span style={{fontSize:11,fontWeight:700,
                    color:timeTab==='slowest'?'var(--red)':'var(--cyan)',
      ...
      <Section title="💡 Smart Insights">
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(230px,1fr))',gap:10}}>
          {[
            ...
            grp.strong>10&&{c:'var(--cyan)',icon:'💪', t:`${grp.strong} mastered`,          d:'Strong foundation. Consider building a portfolio project now.'},
            ...
            !activeDays && {c:'var(--cyan)',icon:'🌱', t:'Day 1 starts now',               d:'Rate your first topic and start a timer. The journey begins!'},
          ].filter(Boolean).slice(0,4).map((ins,i)=>(
            <div key={i} style={{background:'var(--s3)',borderRadius:12,padding:'12px 14px',
