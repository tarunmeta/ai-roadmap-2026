...
const COLORS = { strong:'#10b981', mid:'#f59e0b', weak:'#ef4444', later:'#a855f7' }
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{background:'#0d0f1a',border:'1px solid #242740',borderRadius:8,padding:'8px 12px',fontSize:11}}>
      <div style={{color:'#8b90b0',marginBottom:4}}>{label}</div>
...
function StatCard({ icon, label, value, sub, color='#6366f1', trend }) {
  return (
    <div style={{background:'#0d0f1a',border:'1px solid #1a1d2e',borderRadius:14,padding:'18px 16px',position:'relative',overflow:'hidden',transition:'transform .2s',cursor:'default'}}
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10}}>
        ...
        {trend!==undefined && (
          <div style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:99,
            background:trend>=0?'rgba(16,185,129,.12)':'rgba(239,68,68,.1)',
      ...
      {sub && <div style={{fontSize:10,color:'#454869',marginTop:3}}>{sub}</div>}
    </div>
  )
}
...
function InactivityTracker({ prod }) {
  ...
  const inactivityColor = currentInactivity < 20 ? '#10b981' : currentInactivity < 50 ? '#f59e0b' : '#ef4444'
  ...
  return (
    <div style={{background:'#0d0f1a',border:`1px solid ${inactivityColor}33`,borderRadius:16,padding:'20px',marginBottom:16}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
        ...
        <div style={{display:'flex',gap:10}}>
          <div style={{textAlign:'center',padding:'8px 14px',borderRadius:10,background:`${inactivityColor}10`,border:`1px solid ${inactivityColor}33`}}>
            ...
            <div style={{fontSize:9,color:'#8b90b0',fontWeight:600}}>INACTIVITY</div>
          </div>
          <div style={{textAlign:'center',padding:'8px 14px',borderRadius:10,background:'rgba(99,102,241,.08)',border:'1px solid rgba(99,102,241,.2)'}}>
            <div style={{fontSize:22,fontWeight:900,color:'#818cf8'}}>{streak}</div>
            <div style={{fontSize:9,color:'#8b90b0',fontWeight:600}}>DAY STREAK</div>
          </div>
        </div>
      </div>
      ...
      <div style={{marginBottom:12}}>
        <div style={{fontSize:10,color:'#454869',marginBottom:6}}>Inactivity level — rises when you don't study, drops when you do</div>
        <ResponsiveContainer width="100%" height={100}>
          <AreaChart data={inactivityData}>
            ...
            <XAxis dataKey="label" tick={{fontSize:8,fill:'#454869'}} tickLine={false} axisLine={false} interval={4}/>
      ...
      <div style={{fontSize:10,color:'#454869',marginBottom:6}}>Last 30 days — green = active, red = inactive</div>
        ...
        {days.map((d,i) => (
          <div key={i} title={`${d.label}: ${d.active?`${d.topics} topics, ${d.minutes}min`:'No activity'}`}
            style={{width:20,height:20,borderRadius:4,fontSize:8,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,
              background: d.date===today ? 'rgba(99,102,241,.2)' : d.active ? 'rgba(16,185,129,.25)' : 'rgba(239,68,68,.12)',
              border: `1px solid ${d.date===today?'#6366f144':d.active?'rgba(16,185,129,.3)':'rgba(239,68,68,.2)'}`,
              color: d.date===today?'#818cf8':d.active?'#10b981':'#ef4444',
            }}>
...
function NotebookLMGuide() {
  ...
  return (
    <div style={{background:'#0d0f1a',border:'1px solid rgba(6,182,212,.25)',borderRadius:16,overflow:'hidden',marginBottom:16}}>
      <button onClick={()=>setOpen(o=>!o)}
        style={{width:'100%',padding:'18px 20px',background:'linear-gradient(135deg,rgba(6,182,212,.08),rgba(99,102,241,.05))',
        ...
        <span style={{color:'#8b90b0',fontSize:18,transition:'transform .2s',transform:open?'rotate(180deg)':'rotate(0)'}}>▾</span>
      </button>
      ...
      {open && (
        <div style={{padding:'0 20px 20px',animation:'fadeUp .25s ease'}}>
          ...
          <div style={{padding:'16px',borderRadius:12,background:'rgba(6,182,212,.06)',border:'1px solid rgba(6,182,212,.15)',marginBottom:16,marginTop:16}}>
            ...
            <div style={{fontSize:12,color:'#8b90b0',lineHeight:1.8}}>
          ...
          <div style={{padding:'14px',borderRadius:12,background:'rgba(239,68,68,.06)',border:'1px solid rgba(239,68,68,.15)',marginBottom:16}}>
            ...
            <div style={{fontSize:12,color:'#8b90b0',lineHeight:1.8}}>
              ...
              <em style={{color:'#454869'}}>Tip: Paste multiple video transcripts + a paper on the same topic → ask NotebookLM to connect them.</em>
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:10}}>
            {prompts.map((p,i)=>(
              <div key={i} style={{background:'#08090f',border:'1px solid #1a1d2e',borderRadius:12,padding:'14px',transition:'border-color .15s'}}
                ...
                <div style={{fontSize:11,color:'#8b90b0',lineHeight:1.65,fontStyle:'italic',background:'rgba(255,255,255,.02)',padding:'8px',borderRadius:7,border:'1px solid #1a1d2e'}}>
                  "{p.prompt}"
                </div>
                <button onClick={()=>navigator.clipboard.writeText(p.prompt)}
                  style={{marginTop:8,width:'100%',padding:'5px',borderRadius:7,border:'1px solid #1a1d2e',background:'transparent',color:'#454869',fontSize:10,fontWeight:700,cursor:'pointer',transition:'all .15s'}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor='#06b6d4';e.currentTarget.style.color='#06b6d4'}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor='#1a1d2e';e.currentTarget.style.color='#454869'}}>
                  📋 Copy Prompt
                </button>
              </div>
            ))}
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:6}}>
              {[
                ...
                <a key={title} href={`https://${url}`} target="_blank" rel="noreferrer"
                  style={{display:'block',padding:'10px',borderRadius:9,background:'#0d0f1a',
                  ...
                  <div style={{fontSize:10,color:'#454869'}}>{desc}</div>
                </a>
              ))}
            </div>
          </div>
...
export default function ProductivityDashboard({ done, grasp, times, prod, user }) {
  ...
  const topTopics = useMemo(() => all
    ...
    .slice(0,6)
    .map(t => ({ name: t.text.slice(0,32)+'…', mins: Math.round((times[t.id]||0)/60000), color: COLORS[grasp[t.id]]||'#6366f1' }))
  , [times, grasp])
  ...
  return (
    <div style={{paddingBottom:40}}>
      ...
      <div style={{marginBottom:20,padding:'18px 20px',background:'linear-gradient(135deg,#0d0f1a,#111420)',border:'1px solid #1a1d2e',borderRadius:16,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <div>
            ...
            <div style={{fontSize:12,color:'#8b90b0',marginTop:3}}>
            ...
            {[7,14,30].map(r=>(
              <button key={r} onClick={()=>setRange(r)}
                style={{padding:'5px 12px',borderRadius:8,cursor:'pointer',fontSize:11,fontWeight:700,
                  border:`1px solid ${range===r?'#6366f1':'#242740'}`,
                  background:range===r?'rgba(99,102,241,.15)':'transparent',
                  color:range===r?'#818cf8':'#454869'}}>
                {r}d
              </button>
            ))}
          </div>
      ...
      {!hasData ? (
        <div style={{padding:'48px 24px',textAlign:'center',background:'#0d0f1a',border:'1px solid #1a1d2e',borderRadius:16,marginBottom:16}}>
          <div style={{fontSize:48,marginBottom:14}}>📊</div>
          <div style={{fontSize:17,fontWeight:800,marginBottom:8}}>No data yet</div>
          <div style={{color:'#8b90b0',fontSize:13,lineHeight:1.65}}>Start completing topics and the dashboard will fill up.<br/>Your progress, streaks, and study time will appear here.</div>
        </div>
      ) : (
        <>
          {/* Charts row */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12,marginBottom:12}}>
            {/* Topics per day */}
            <div style={{background:'#0d0f1a',border:'1px solid #1a1d2e',borderRadius:14,padding:'16px 14px'}}>
              ...
              <ResponsiveContainer width="100%" height={130}>
                <AreaChart data={chartData}>
                  ...
                  <XAxis dataKey="label" tick={{fontSize:8,fill:'#454869'}} tickLine={false} axisLine={false} interval={Math.floor(range/5)}/>
                  <Tooltip content={<CustomTooltip/>}/>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:5}}>
                    {graspData.map(d=>(
                      <div key={d.name} style={{display:'flex',alignItems:'center',gap:6}}>
                        ...
                        <span style={{fontSize:11,color:'#8b90b0'}}>{d.name}</span>
                  ...
                  {topTopics.map((t,i)=>(
                    <div key={i}>
                      <div style={{display:'flex',justifyContent:'space-between',fontSize:10,marginBottom:4}}>
                        <span style={{color:'#8b90b0',maxWidth:'75%',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.name}</span>
