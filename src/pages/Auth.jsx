        ...
        return (
          <input key={i} ref={el=>refs.current[i]=el}
            ...
            style={{
              ...
              background:d?'rgba(0,229,255,.07)':'var(--s2)',
        ...
        <div key={i} style={{display:'flex',alignItems:'center',flex:i<steps.length-1?1:'initial'}}>
          <div style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
            <div style={{
              ...
              background: step>i+1?'var(--cyan)':step===i+1?'linear-gradient(135deg,var(--cyan),var(--violet))':'var(--s3)',
              color: step>=i+1?'var(--void)':'var(--txt3)',
          ...
          {i<steps.length-1&&(
            <div style={{flex:1,height:1,background:step>i+1?'var(--cyan)':'var(--ln)',margin:'0 8px',marginBottom:14,transition:'background .3s'}}/>
          )}
        </div>
      ))}
    </div>
          ...
          {stepsUI.map((s,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
              <div style={{
                ...
                background:step>s.num?'var(--cyan)':step===s.num?'linear-gradient(135deg,var(--cyan),var(--violet))':'var(--s3)',
                color:step>=s.num?'var(--void)':'var(--txt3)',
                border:`2px solid ${step>=s.num?'transparent':'var(--line2)'}`,
                transition:'all .3s', flexShrink:0,
              }}>{step>s.num?'✓':s.num}</div>
              {i<2&&<div style={{width:32,height:1,background:step>s.num?'var(--cyan)':'var(--ln)',transition:'background .3s'}}/>}
            </div>
          ))}
        </div>
