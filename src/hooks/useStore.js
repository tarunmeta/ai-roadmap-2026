...
const lk  = (uid,k) => uid ? `ai_rm_${uid}_${k}` : `ai_rm_guest_${k}`
...
export function useStore(uid = null) {
  const isGuest = !uid
  ...
  useEffect(() => {
    ...
    // If logging in (null→uid), migrate guest localStorage to user key
    if (uid && !prevUid) {
      const guestDone  = lget(lk(null,'done'))
      const guestGrasp = lget(lk(null,'grasp'))
      const guestNotes = lget(lk(null,'notes'))
      const guestTimes = lget(lk(null,'times'))
      if (Object.keys(guestDone).length > 0) {
        lset(lk(uid,'done'),  guestDone)
        lset(lk(uid,'grasp'), guestGrasp)
        lset(lk(uid,'notes'), guestNotes)
        lset(lk(uid,'times'), guestTimes)
      }
    }
    // Reload state from localStorage for new uid
    setDone(lget(lk(uid,'done')))
    setGrasp(lget(lk(uid,'grasp')))
    setNotes(lget(lk(uid,'notes')))
    setTimes(lget(lk(uid,'times')))
    setTimer(lget(lk(uid,'timer')))
    setProd(lget(lk(uid,'prod')))
  }, [uid])
